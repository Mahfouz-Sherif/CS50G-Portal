...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        if not self.launchMarker.playerSplit then
            ...
            for k, alien in pairs(self.launchMarker.alien) do -- iterate over each alien in alien table
                local xPos, yPos = alien.body:getPosition()
                local xVel, yVel = alien.body:getLinearVelocity()
                
                -- if split alien is to the left or it's almost done rolling, remove it from alien table
                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then 
                    alien.body:destroy()
