...
function AlienLaunchMarker:init(world)
    ...
    -- collision flag for our player by default it is false as the player hasn't collided with anything yet
    self.playerCollided = false  
    -- split flag to disable splitting more than once
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if not self.playerCollided and love.keyboard.wasPressed('space') and not self.playerSplit then  --check if player hasn't collided and spacebar was pressed
            --paused = true --used to test the condition
            self:split()
...
function AlienLaunchMarker:split()
    local alienToSplit = self.alien
   
    -- using alienToSplit.body:getUserData() in Alien constructors produces an error in class:level line 25 table index is nil

    local alien1 = Alien(alienToSplit.body:getWorld(), alienToSplit.body:getType(), alienToSplit.body:getX(), alienToSplit.body:getY(), 'Player')
    alien1.body:setLinearVelocity(alienToSplit.body:getLinearVelocity())

    alien1.fixture:setRestitution(0.4)
    alien1.body:setAngularDamping(1)

    local alien2 = Alien(alienToSplit.body:getWorld(), alienToSplit.body:getType(), alienToSplit.body:getX() - 30, alienToSplit.body:getY() - 30, 'Player')
    alien2.body:setLinearVelocity(alienToSplit.body:getLinearVelocity())

    alien2.fixture:setRestitution(0.4)
    alien2.body:setAngularDamping(1)

    local alien3 = Alien(alienToSplit.body:getWorld(), alienToSplit.body:getType(), alienToSplit.body:getX() -30, alienToSplit.body:getY()+ 30, 'Player')
    alien3.body:setLinearVelocity(alienToSplit.body:getLinearVelocity())

    alien3.fixture:setRestitution(0.4)
    alien3.body:setAngularDamping(1)

    self.alien.body:destroy()
    self.alien ={} -- changed to an empty table to add new aliens in it
